 ...
 * redirect_uri when sending a user to LeadsAuth for an interactive login.
 *
 * An optional session ID and host may be provided if there is desire for
 * LeadsAuth to be aware of this information.  They may be used by LeadsAuth
 * when session invalidation is triggered from the LeadsAuth console itself
 * during its postbacks to `/la_logout` on the server.
 *
 * This method returns or promise or may optionally take a callback function.
 *
 * @param {String} code The code from a successful login redirected from LeadsAuth.
 * @param {String} sessionId Optional opaque session-id.
 * @param {String} sessionHost Optional session host for targetted LeadsAuth console post-backs.
 * @param {Function} callback Optional callback, if not using promises.
 */
...
/**
 * Perform live validation of an `access_token` against the LeadsAuth server.
 *
...
GrantManager.prototype.userInfo = function userInfo (token, callback) {
  ...
  options.headers = {
    ...
    'X-Client': 'leadsauth-nodejs-utils'
  };
 ...
 *
 * @param {String} rawData The raw JSON string received from the LeadsAuth server or from a client.
